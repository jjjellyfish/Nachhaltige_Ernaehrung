...
```{r functions}
rsquared_aov <- function(model) {
  aov_result <-  aov(eval(parse(text = model)), data=v2)
...
### 'Nullmodell' mit kontinuierlichem Score
model <- paste("score_v1 ~ ", kov_string)
r2 <- rsquared_aov(model)

### Cutoff-Modelle
### Verteilungsbasiert: 1. Quintile
model <- paste("co_m1 ~ ", kov_string) 
r2 <- c(r2, rsquared_aov(model))

## 2. Quartile
model <- paste("co_m2 ~ ", kov_string) 
r2 <- c(r2, rsquared_aov(model))

## 3. Terzile
model <- paste("co_m3 ~ ", kov_string) 
r2 <- c(r2, rsquared_aov(model))

## 4. Quintile mit kleinen Randgruppen
model <- paste("co_m4 ~ ", kov_string) 
r2 <- c(r2, rsquared_aov(model))

## 5. Qauartile mit kleinen Randgruppen
model <- paste("co_m5 ~ ", kov_string) 
r2 <- c(r2, rsquared_aov(model))

### Score-basiert
## 6. Quintile max. Score
model <- paste("co_m6 ~ ", kov_string) 
r2 <- c(r2, rsquared_aov(model))

## 7. Quartile max. Score
model <- paste("co_m7 ~ ", kov_string) 
r2 <- c(r2, rsquared_aov(model))

## 8. Terzile max. Score
model <- paste("co_m8 ~ ", kov_string) 
r2 <- c(r2, rsquared_aov(model))

## 9. Quartile/Terzile max. Score
model <- paste("co_m9 ~ ", kov_string) 
r2 <- c(r2, rsquared_aov(model))
```


#### ordinale Modelle
UCLA-Guide: <https://stats.oarc.ucla.edu/r/dae/ordinal-logistic-regression/>

```{r}
#| label: cutoff_ordinale_modelle
v2 <- v2 %>% mutate_at(vars(contains('co_m')), as.factor)

model <- paste("co_m1 ~ ", kov_string)
ord_null <-  MASS::polr(co_m1 ~ 1, data=v2, Hess=TRUE)
ord_result <-  MASS::polr(eval(parse(text = model)), data=v2, Hess=TRUE)
...
pR2_nagelkerke(v2$co_m1, v2)


ord_result <-  rms::lrm(eval(parse(text = model)), data=v2)
print(ord_result)

pR2_nagelkerke <- as.numeric(ord_result$stats["R2"])

pR2_mcfadden = 1 - ord_result$deviance[2] / ord_result$deviance[1]

R2Measures(ord_result)
