...
## 2.4 EFA: Einfaktorielles Modell

```{r}
#|label: cor_matrix3
tmp <- sust %>% 
  select(lfdn, v_321, v_329, v_335, v_346, v_347, v_348, v_349, v_350, v_364, ern_erw) %>% 
  na.omit()
...
```{r}
#| label: efa3
efa <- psych::fa(r=tmp[,c(-1)], nfactors=1,
 max.iter=100,
 rotate='promax')
...
write.xlsx(efa_df, file='data/EFA_loadings3.xlsx')
```
...
v1 <- tmp %>% select(lfdn, v_321, v_325, v_329, v_330, v_331, v_332, v_334, v_335, v_346, v_347, v_348, v_349, v_350, v_351, v_359, v_360, v_364, v_365, ern_erw, ern_kid)

v2 <- tmp %>% select(lfdn, v_321, v_329, v_335, v_346, v_347, v_348, v_349, v_350, v_364, ern_erw)
```

```{r}
#| label: scores
v1 <- v1 %>% mutate(score_v1=rowSums(select(., 2:length(v1)), na.rm=TRUE)/(length(v1)-1))
v2 <- v2 %>% mutate(score_v2=rowSums(select(., 2:length(v2)), na.rm=TRUE)/(length(v2)-1))

mosaic::favstats(v1$score_v1)
mosaic::favstats(v2$score_v2)
```
...
ggplot(v2, aes(x=score_v2)) +
  geom_histogram(binwidth=0.005, fill="#69b3a2", color="#e9ecef", alpha=0.9) +
  ggtitle("Verteilung der Scores - Version 2") +
  theme_bw() +
  theme(plot.title = element_text(size=15))
```
