...
```{r transformation}
table(sust$v_325, useNA = 'always')

tmp <- sust

#X = (x - 1) / (b - a)
tmp$v_325_tr <- (tmp$v_325 - 1) / (5 - 1)
table(tmp$v_325_tr, useNA = 'always')
mean(tmp$v_325_tr, na.rm=TRUE)

tmp$v_325_tr <- (tmp$v_325 - min(tmp$v_325, na.rm=TRUE)) / (5 - min(tmp$v_325, na.rm=TRUE))
