...
models <- c('Original', '1. Quintile', '2. Quartile', '3. Terzile', '4. Quintile mit kleinen Randgruppen', '5. Quartile mit kleinen Randgruppen', '6. Quintile max. Score', '7. Quartile max. Score', '8. Terzile max. Score', '9. Quartile/Terzile max. Score')
```
...
tmp %>% 
  kbl(booktabs = T,  digits = 3, centering=F, row.names=FALSE, 
      col.names=c('Score / Modell', 'Adjusted R²', 'McFadden', 'Nagelkerke', 'Nachhaltigkeits-Score'), 
      caption = 'Aufgeklärte Varianz in Regressionsmodellen und Korrelationen mit dem kontinuierlichen Nachhaltigkeits-Score') %>%
  add_header_above(c('', 'Lineare Regression'=1, 'Ordinale Regression\n Pseudo R²'=2, 'Spearman-Rho\n Korrelation'=1)) %>%
  kable_styling(position = "left") %>% 
...
```{r}
#| label: cutoff_modellbewertung_plot
#### Verteilungsbasiert
## Modell 2
#tabyl(v2$co_int2)
ggplot(v2, aes(x=score_v1)) +
  geom_histogram(binwidth=0.005, fill="#BED600", color="#e9ecef", alpha=0.9) +
  ggtitle("Cutoffs Modell 2: vertilungsbasiert, Quartile") +
...
ggplot(v2, aes(x=score_v1)) +
  geom_histogram(binwidth=0.005, fill="#BED600", color="#e9ecef", alpha=0.9) +
  ggtitle("Cutoffs Modell 3: verteilungsbasiert, Terzile") +
  xlab("kontinuierlicher Nachhaltigkeits-Score") + 
  ylab("Häufigkeit") +
  theme_bw() +
  theme(plot.title = element_text(size=15)) +
  geom_vline(xintercept = c(0.501, 0.609), linetype="dotted", color = "#bd182d", size=1)


#### Basiert auf maximalem Score
## Modell 6
ggplot(v2, aes(x=score_v1)) +
  geom_histogram(binwidth=0.005, fill="#BED600", color="#e9ecef", alpha=0.9) +
...
ggplot(v2, aes(x=score_v1)) +
  geom_histogram(binwidth=0.005, fill="#BED600", color="#e9ecef", alpha=0.9) +
  ggtitle("Cutoffs Modell 6: max. Score basiert, Quartile") +
  xlab("kontinuierlicher Nachhaltigkeits-Score") + 
  ylab("Häufigkeit") +
  theme_bw() +
  theme(plot.title = element_text(size=15)) +
