...
model1 <- paste("score_v1 ~ ", kov_string) 

anv0 <-  aov(eval(parse(text = model1)), data=v2)
summary(anv0)


# ab hier BAUSTELLE
lm0 <-  lm(eval(parse(text = model1)), data=v2)
summary(lm0)

## to do: in Funktion packen
library(broom)
tidy_aov <- tidy(anv0)
...
# Aber auch: Wert scheint viel zu hoch: Rechnung stimmt nicht
```
