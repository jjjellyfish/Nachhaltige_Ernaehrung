...
Da die in der Skala verwendeten Items zum Teil sehr unterschiedliche Aspekte der nachhaltigen Ernährung abbilden, wird für die Berechnung eines Nachhaltigkeits-Scores ein hoher Anteil an valide beantwortet Items vorausgesetzt. Der Schwellenwert wird von auf 90% (18 von 20 Items) festgelegt, weil so einerseits gut gewährleistet werden kann, dass möglichst viele Aspekte der Nachhaltigkeit im Score Berücksichtigung finden und andrerseits die Berechnung eines Scores noch immer für den Großteil der Stichprobe erfolgen kann. 
...
```{r}
#Kategrien benennen
v2 <- v2 %>% mutate(co_m9_cat = case_when(
...
ggplot(v2, aes(x="", y=co_m9_cat, fill=co_m9_cat)) +
  geom_bar(stat="identity") + 
  coord_polar(theta="y") +
  scale_fill_manual(values=c("#00386a", "#BED600", "#eda63b", '#cbd3b2')) +
  guides(fill=guide_legend("Bedeutung von Nachhaltigkeit")) + 
  theme_void()
```


Es ergibt sich eine Verteilung nach der etwa 10% der Stichprobe der nachhaltigen Ernährung nur wenig Bedeutung beimessen. Für etwas mehr als die Hälfte ist Nachhaltigkeit in der Ernährung mäßig bedeutsam. Etwa 30% messen der Nachhaltigkeit eine hohe und 7% eine sehr hohe Bedeutung zu. 
